...
import { getCell, getFog, getPlayerProcessor, pointRange, renderText, TerminalState, wrapCoordinates } from "../../engine/utils";
    ...
    <div className="Viewport">
      {state.hp <= 0 ? (
        <>
          ...
          <div className="Row" />
          <div className="Row">
            {renderText('   You survived for')}
          </div>
          <div className="Row">
            {renderText(`    ${Math.floor(state.tick / 3).toString().padStart(5)} seconds`)}
          </div>
        </>
      ) : (
